...
class UserService:
    ...
    async def update_social_links_from_user(self, id: str|None, username: str|None, social_links_data: SocialLinksUpdateSchema) -> dict[str, int]:
        user = await self.find_user_document(id, username)
        social_links_update_result = self.__social_links_service.update_social_links_document(user, social_links_data)
        return social_links_update_result
    
    async def find_social_links_from_user(self, id: str|None, username: str|None) -> dict[str, str]:
        user = await self.find_user_document(id, username)
