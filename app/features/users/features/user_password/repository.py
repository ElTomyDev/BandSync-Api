...
class UserPasswordRepository:
    def __init__(self, request: Request):
        self.__collection = request.app.state.db['user_passwords']
    
    async def insert_one(self, password_model: UserPasswordModel) -> None:
        await self.__collection.insert_one(password_model.model_dump())
    
    async def find_by_user_id(self, user_id: ObjectId) -> UserPasswordModel | None:
        password = await self.__collection.find_one({'user_id': user_id})
